...
// const moneyRaised = GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0);
const totalGames = GAMES_JSON.length;
//const unfundedGamesNum = GAMES_JSON.filter(game => game.pledged < game.goal).length;

const displayStr = `A total of $${totalRaised.toLocaleString()} has been raised for ${totalGames} games. Currently, ${numUnfundedGames}
...
const firstPledgeElement = document.createElement('p');
firstPledgeElement.textContent = `Pledged: $${firstGame.pledged.toLocaleString()}`;
...
secondPledgeElement.textContent = `Pledged: $${secondGame.pledged.toLocaleString()}`;
